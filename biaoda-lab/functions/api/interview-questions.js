// 📎 Cloudflare Pages Functions 版本 · 逻辑与 api/interview-questions.js（Vercel 版）保持一致
// 📍 两版需同步维护：System Prompt / 出题侧重池 / 校验规则不允许出现差异。
//
// 表达研究所 · 完整面试题目生成（后端代理）
...
export async function onRequestPost({ request, env }) {
  ...
  if (!jdText) {
    return json({ status: 'invalid_input', error_type: 'empty_jd', message: '请先粘贴岗位 JD（职位描述）' });
  }
  if (jdText.length < 30) {
    return json({ status: 'invalid_input', error_type: 'jd_too_short', message: 'JD 内容太短了，请粘贴完整的职位描述（至少 30 字）' });
  }
  if (jdText.length > 5000) {
    return json({ status: 'invalid_input', error_type: 'jd_too_long', message: 'JD 内容过长，请精简到 5000 字以内' });
  }
